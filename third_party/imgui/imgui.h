...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current window pos in screen space (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
    ...
    IMGUI_API void          SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0, const ImVec2& pivot = ImVec2(0, 0)); // set next window pos. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set current window pos - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    ...
    IMGUI_API void          SetWindowPos(const char* name, const ImVec2& pos, ImGuiCond cond = 0);      // set named window pos.
    ...
    IMGUI_API void          SetScrollHereX(float center_x_ratio = 0.5f);                    // adjust scrolling amount to make current cursor pos visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollHereY(float center_y_ratio = 0.5f);                    // adjust scrolling amount to make current cursor pos visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollFromPosX(float local_x, float center_x_ratio = 0.5f);  // adjust scrolling amount to make given pos visible. Generally GetCursorStartPos() + offset to compute a valid pos.
    IMGUI_API void          SetScrollFromPosY(float local_y, float center_y_ratio = 0.5f);  // adjust scrolling amount to make given pos visible. Generally GetCursorStartPos() + offset to compute a valid pos.
    ...
    IMGUI_API float         CalcItemWidth();                                                // width of item given pushed settings and current cursor pos. NOT necessarily the width of last item unlike most 'Item' functions.
    IMGUI_API void          PushTextWrapPos(float wrap_local_pos_x = 0.0f);                 // push word-wrapping pos for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' pos in window local space
    ...
    // - By "cursor" we mean the current output pos.
    // - The typical widget behavior is to output themselves at the current cursor pos, then move the cursor one line down.
    ...
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor pos, absolute coordinates. THIS IS YOUR BEST FRIEND (prefer using this rather than GetCursorPos(), also more useful to work with ImDrawList API).
    IMGUI_API void          SetCursorScreenPos(const ImVec2& pos);                          // cursor pos, absolute coordinates. THIS IS YOUR BEST FRIEND.
    IMGUI_API ImVec2        GetContentRegionAvail();                                        // available space from current pos. THIS IS YOUR BEST FRIEND.
    IMGUI_API ImVec2        GetCursorPos();                                                 // [window-local] cursor pos in window-local coordinates. This is not your best friend.
    ...
    IMGUI_API ImVec2        GetCursorStartPos();                                            // [window-local] initial cursor pos, in window-local coordinates. Call GetCursorScreenPos() after Begin() to get the absolute coordinates version.

    // Other layout functions
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a menu bar or in horizontal layout mode, this becomes a vertical separator.
    IMGUI_API void          SameLine(float offset_from_start_x=0.0f, float spacing=-1.0f);  // call between widgets or groups to layout them horizontally. X pos given in window coordinates.
    ...
    IMGUI_API void          Indent(float indent_w = 0.0f);                                  // move content pos toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                // move content pos back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          BeginGroup();                                                   // lock horizontal starting pos
    IMGUI_API void          EndGroup();                                                     // unlock horizontal starting pos + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    ...
    IMGUI_API void          Bullet();                                                       // draw a small circle + keep the cursor on the same line. advance cursor x pos by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
    ...
    IMGUI_API float         GetColumnOffset(int column_index = -1);                             // get pos of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
    IMGUI_API void          SetColumnOffset(int column_index, float offset_x);                  // set pos of column line (in pixels, from the left side of the contents region). pass -1 to use current column
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if rectangle (of given size, starting from cursor pos) is visible / not clipped.
    ...
    // - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking pos (see 'lock_threshold' and 'io.MouseDraggingThreshold')
    ...
    IMGUI_API ImVec2        GetMousePosOnOpeningCurrentPopup();                                 // retrieve mouse pos at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
    ...
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // return the delta from the initial clicking pos while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (uses io.MouseDraggingThreshold if lock_threshold < 0.0f)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 20,  // Callback on each iteration. User code may query cursor pos, modify text buffer.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_DrawLinesFull        = 1 << 19,  // Horizontal lines to child nodes. Vertical line drawn down to TreePop() pos: cover full contents. Faster (for large trees).
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_Leading                       = 1 << 6,   // Enforce the tab pos to the left of the tab bar (after the tab list popup button)
    ImGuiTabItemFlags_Trailing                      = 1 << 7,   // Enforce the tab pos to the right of the tab bar (before the scrolling buttons)
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_AllowWhenOverlappedByWindow   = 1 << 9,   // IsItemHovered() only: Return true even if the pos is obstructed or overlapped by another window.
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceAllowNullID            = 1 << 3,   // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by manufacturing a temporary identifier based on their window-relative pos. This is extremely unusual within the dear imgui ecosystem and so we made it explicit.
...
enum ImGuiKey : int
{
    ...
    ImGuiKey_GamepadFaceDown,       // A (Xbox)         B (Switch)   Cross (PS)         // use / Open / Toggle / Tweak
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_HasSetMousePos        = 1 << 2,   // Backend Platform supports io.WantSetMousePos requests to reposition the OS mouse pos (only used if io.ConfigNavMoveSetMousePos is set).
...
struct ImGuiStyle
{
    ...
    ImVec2      TouchExtraPadding;          // Expand reactive bounding box for touch-based system where touch pos is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
...
struct ImGuiIO
{
    ...
    bool        ConfigNavSwapGamepadButtons;    // = false          // Swap use<>Cancel (A<>B) buttons, matching typical "Nintendo/Japanese style" gamepad layout.
    ...
    IMGUI_API void  AddMousePosEvent(float x, float y);                     // Queue a mouse pos update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)
    ...
    ImVec2      MouseDelta;                         // Mouse delta. Note that this is zero if either current or previous pos are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't have a huge delta.
    ...
    ImVec2      MousePos;                           // Mouse pos, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)
    ...
    ImVec2      MousePosPrev;                       // Previous mouse pos (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either pos is invalid)
...
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  Pos;            // Read-only.   Window pos, for reference.
    ImVec2  CurrentSize;    // Read-only.   Current window size.
    ImVec2  DesiredSize;    // Read-write.  Desired size, based on user's mouse pos. Write to this field to restrain resizing.
};
...
// - Clipper calculate the actual range of elements to display based on the current clipping rectangle, pos the cursor before the first visible element.
...
struct ImGuiListClipper
{
    ...
    float           StartPosY;          // [Internal] Cursor pos at the time of Begin() or after table frozen rows are all processed
...
struct ImDrawData
{
    ...
    ImVec2              DisplayPos;         // Top-left pos of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)
...
{
    unsigned short  X, Y;           // Output   // Packed pos in Atlas
...
struct ImFontAtlas
{
    ...
    // - After calling Build(), you can query the rectangle pos and render your pixels.
...
struct ImGuiViewport
{
    ...
    bool                PlatformRequestMove;    // Platform window requested move (e.g. window was moved by the OS / host window manager, authoritative pos will be OS window pos)
...
// - So e.g. ImGui::SetNextWindowPos(ImVec2(0,0)) will pos a window relative to your primary monitor!
// - If you want to pos windows relative to your main application viewport, use ImGui::GetMainViewport()->Pos as a base pos.
//
...
struct ImGuiPlatformIO
{
    ...
    // Optional: Notify OS Input Method Editor of the screen pos of your cursor for text input pos (e.g. when using Japanese/Chinese IME on Windows)
    ...
    //   N = ImGui::NewFrame()                        ~ beginning of the dear imgui frame: read info from platform/OS windows (latest size/pos)
    ...
    void    (*Platform_SetWindowPos)(ImGuiViewport* vp, ImVec2 pos);        // . . U . .  // Set platform window pos (given the upper-left corner of client area)
...
// We use this information for multiple DPI support + clamping the pos of popups and tooltips so they don't straddle multiple monitors.
